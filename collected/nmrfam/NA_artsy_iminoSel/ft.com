...
set tauValues = ( 5.75 11.5 )    # evolution times for each QJ point
...
#  set ucsfName = (`printf ./dnh_t%03d.ucsf $n`)
  ...
  | nmrPipe  -fn FT                                     \
  | nmrPipe  -fn EXT -x1 16.0ppm -xn 9.0ppm -sw -verb	\
  | nmrPipe  -fn PS -p0 116 -p1 -34.2 -di		\
  | nmrPipe  -fn TP                                     \
  | nmrPipe  -fn SP -off 0.5 -end 1.0 -pow 1 -c 0.5	\
  | nmrPipe  -fn ZF -zf 2 -auto				\
  | nmrPipe  -fn FT -verb				\
  | nmrPipe  -fn PS -p0 86.8 -p1 0.0 -di		\
  | nmrPipe  -fn POLY -ord 0 -auto			\
 # | nmrPipe  -fn CS -ls 1.0ppm -sw			\
  | nmrPipe  -fn TP                                     \
  | nmrPipe  -fn POLY -ord 0 -auto			\
     -verb -ov -out $outName
