...
#
# title:   get_jch.py
# summary: Measure analytical JNH values from ratios in nlin.tab
...
def usage():
    print 'usage: get_jch.py [-n <noise>] [-T <time>] <nlin.tab>'
    sys.exit()
...
def help():
    ...
    print """
    This script extracts JNH values from the nlin.tab file created by
...
def main(tabf, bigT, noise, ignore, out=sys.stdout):
    ...
    for l in lines:
        ...
        if l and l[0] != '#':
            ...
            #jch = -2.0/math.pi/bigT*math.acos(0.5*ratio)
            jch = 2.0/math.pi/bigT*math.acos(0.5*ratio)

            err_ratio = (1.0 + ratio*ratio)/(4.0 - ratio*ratio)
            err = 2.0*fnoise/math.pi/bigT*math.sqrt(err_ratio)

            result.append((res, jch, err, toks[idxAssign]))

    result.sort()

    if out != None:
#   	print 'INDEX   J(+D)  ERROR   ASS'
   	print 'VARS INDEX J ERR ASS'
   	print 'FORMAT %4d   %6.1f   %4.1f   %s'

        for res, jch, err, ass in result:
            print '%4i   %4.1f   %4.1f   %s' % (res, jch, err, ass)
